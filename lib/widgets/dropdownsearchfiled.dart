...
class DropdownSearchField extends FormField<String> {
  final dynamic value;
  final Widget icon;
...
class DropDownFieldState extends FormFieldState<String> {
  ...
  @override
  DropdownSearchField get widget => super.widget;
  ...
  List<ListTile> _getChildren(List<Map<String, dynamic>> items) {
    List<ListTile> childItems = List();
    for (var item in items) {
      if (_searchText.isNotEmpty) {
        if (item[widget.titleKey].toUpperCase().contains(
            _searchText.toUpperCase()))
  ...
  ListTile _getListTile(Map<String, dynamic> map) {
    return ListTile(
      dense: true,
      title: Text(
        map[widget.titleKey] ?? "",
      ...
      onTap: () {
        setState(() {
          _effectiveController.text = map[widget.titleKey];
          _handleControllerChanged();
          _showdropdown = false;
          _isSearching = false;
          if (widget.onValueChanged != null) {
            widget.onValueChanged(map);
